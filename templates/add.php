...
<form class="form form--add-lot container <?=!empty($errors) ? 'form--invalid' : ""?>" action="add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
    ...
    <div class="form__container-two">
        <div class="form__item <?=isset($errors['name']) ? "form__item--invalid" : ""?>"> <!-- form__item--invalid -->
            ...
            <input id="lot-name" type="text" name="name" placeholder="Введите наименование лота" value="<?php if (isset($lot)) echo $lot['name']?>">
            <span class="form__error"><?=$errors['name']?></span>
        </div>
        <div class="form__item <?=isset($errors['category']) ? "form__item--invalid" : ""?>">
            <label for="category">Категория <sup>*</sup></label>
            <select id="category" name="category" value="<?php if (isset($lot)) echo $lot['category']?>">
    ...
    <div class="form__item form__item--wide <?=isset($errors['descr']) ? "form__item--invalid" : ""?>">
        ...
        <textarea id="message" name="descr" placeholder="Напишите описание лота" value="<?php if (isset($lot)) echo $lot['descr']?>"></textarea>
        <span class="form__error"><?=$errors['descr']?></span>
    </div>
    ...
    <div class="form__container-three <?=isset($errors['price']) ? "form__item--invalid" : ""?>">
        <div class="form__item form__item--small">
            ...
            <input id="lot-rate" type="text" name="price" placeholder="0" value="<?php if (isset($lot)) echo $lot['price']?>">
        ...
        <div class="form__item form__item--small <?=isset($errors['step']) ? "form__item--invalid" : ""?>">
            ...
            <input id="lot-step" type="text" name="step" placeholder="0" value="<?php if (isset($lot)) echo $lot['step']?>">
            <span class="form__error"><?=$errors['step']?></span>
        </div>
        <div class="form__item <?=isset($errors['dt_end']) ? "form__item--invalid" : ""?>">
            <label for="lot-date">Дата окончания торгов <sup>*</sup></label>
            <input class="form__input-date" id="lot-date" type="text" name="dt_end" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?php if (isset($lot)) echo $lot['dt_end']?>">
